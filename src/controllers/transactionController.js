...
module.exports = {
  ...
  getAgentBankingLimit: async (req, res) => {
    ...
    try {
      const data = await transactionService.fetchAgentBankingLimit(agentId);
      res.status(200).json(data);
    } catch (error) {
      res.status(error.status || 500).json({ message: error.message });
    }
  ...
  getCumulativeCashLimit: async (req, res) => {
    ...
    try {
      const data = await transactionService.fetchCumulativeCashLimit(agentId1);
      res.status(200).json(data);
    } catch (error) {
      res.status(error.status || 500).json({ message: error.message });
    }
  ...
  getMerchantCompliance: async (req, res) => {
    ...
    try {
      const data = await transactionService.fetchMerchantCompliance(merchantId);
      res.status(200).json(data);
    } catch (error) {
      res.status(error.status || 500).json({ message: error.message });
    }
  ...
  getAgentTopCashInCashOut: async (req, res) => {
    const { agentId } = req.params;
    try {
      const data = await transactionService.fetchAgentTopCashInCashOut(agentId);
      res.status(200).json(data);
    } catch (error) {
      res.status(error.status || 500).json({ message: error.message });
    }
