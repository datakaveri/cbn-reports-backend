/*
  Warnings:

  - You are about to drop the column `TransactionType` on the `POSAggregate` table. All the data in the column will be lost.
  - Added the required column `transactionType` to the `POSAggregate` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE "POSAggregate" DROP COLUMN "TransactionType",
ADD COLUMN     "transactionType" TEXT NOT NULL;
